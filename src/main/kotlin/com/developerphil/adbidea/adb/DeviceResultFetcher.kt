...
class DeviceResultFetcher constructor(private val project: Project, private val useSameDevicesHelper: UseSameDevicesHelper, private val bridge: Bridge) {
    ...
    fun fetch(needChoseMoudle:Boolean): DeviceResult? {
        val facets = getApplicationFacets(project)
        if (!facets.isEmpty()) {
            val facet = getFacet(facets,needChoseMoudle) ?: return null
    ...
    private fun getFacet(facets: List<AndroidFacet>,needChoseMoudle:Boolean): AndroidFacet? {
        val facet: AndroidFacet?
        if (facets.size > 1 && needChoseMoudle) {
